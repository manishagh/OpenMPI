  ...
  if(rank==0)
{
   ...
   int done=1;
    int deadWorkers=0;
   // int batch=100;
    int dispRows[disp_width*disp_height/(size-1)];
    int slave;
    int rowNo; 
    FILE *f;
    char str[256];
  ...
  // MPI_Wait(&request,&status);
        ...
        r+=disp_height/(size-1);
        }

   j=disp_height/(size-1);
 for(i=1;i<size;i++)
      {


           gettimeofday(&t3,NULL);
           MPI_Recv(&dispRows,disp_width*disp_height/(size-1),MPI_INT,i,1,MPI_COMM_WORLD, &status);
          ...
          for(x=i*j;x<i*j+j;x++)
                {
                  for(y=0;y<disp_width;y++)
                    {
                        pic[y][x]=dispRows[y+(x-i*j)*disp_width];
                    }
                }
      }
...
int batch=disp_height/(size-1);
int dispRow[disp_width*batch];
int r=0;
int count;
          ...
          count=0;
           do{
               ...
               zimag=2*zreal*zimag+cimag;
                ...
                dispRow[x+k*disp_width]=count; 
         }
      }

        MPI_Send(&dispRow,disp_width*batch,MPI_INT,0,1,MPI_COMM_WORLD);
}
MPI_Finalize();
}